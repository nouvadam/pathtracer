...
impl Ppm {
                ...
                iproduct!((pixel.0 - 1)..=(pixel.0 + 1), (pixel.1 - 1)..=(pixel.1 + 1))
                    .map(|offset_pixel| {
                        self.get_pixel(offset_pixel.0, offset_pixel.1)
                            .unwrap_or_else(|| panic!("{} {}", offset_pixel.0, offset_pixel.1))
                    })
    ...
    pub fn median_filter(&self, size: u32) -> Ppm {
        ...
        iproduct!(size..(self.width - size), size..(self.height - size)).for_each(|pixel| {
            // Create matrix window with this pixel in the center
            let matrix: Vec<V3<u8>> = iproduct!(
                (pixel.0 - size)..=(pixel.0 + size),
                (pixel.1 - size)..=(pixel.1 + size)
            )
            .map(|offset_pixel| {
                self.get_pixel(offset_pixel.0, offset_pixel.1)
                    .unwrap_or_else(|| panic!("{} {}", offset_pixel.0, offset_pixel.1))
            ...
            let medians: Vec<u8> = matrix
                ...
                .multizip()
                .map(|mut vec| quickselect(&mut vec, mid as usize))
                .collect();

            // Insert new pixel values from medians for each channel
            copied_self.set_pixel(
                pixel.0,
                pixel.1,
                V3::new(medians[0], medians[1], medians[2]),
            );
        });
...
mod tests {
    ...
    fn median_filter_test() {
        let before_filter = vec![
            1, 0, 0, 1, 4, 0, 1, 1, 0, 0, 7, 0, 0, 3, 255, 1, 5, 0, 0, 2, 0, 0, 8, 0, 1, 6, 0,
        ];
        let correct_filter = vec![
            1, 0, 0, 1, 4, 0, 1, 1, 0, 0, 7, 0, 1, 4, 0, 1, 5, 0, 0, 2, 0, 0, 8, 0, 1, 6, 0,
        ];
        let ppm_after_filter = Ppm::new_from_vec(3, 3, before_filter.clone())
            .unwrap()
            .median_filter(1);
